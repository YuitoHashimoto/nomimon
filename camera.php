...
print $id;
   ...
   if(ISSET($_POST['submitQR'])){
      ...
      $result = $db->query("SELECT stylish,cute,cool FROM userData WHERE id='{$id}'");
      ...
      $query = $db->query("UPDATE userData SET stylish='{$newStylish}',cute='{$newCute}',cool='{$newCool}' WHERE id='{$id}';");
      if($query) {
            $result = $db->query("SELECT `stylish`, `cute`, `cool` FROM `userData` WHERE id = '{$id}' ");
            if($result -> rowCount()){
               foreach($result as $row){
            ...
            if($row["stylish"] > 100 || $row["cute"] >100 || $row["cool"] > 100){
               $db->query("UPDATE `userData` SET `animal`='dog' WHERE `ID` = '{$id}'");
               print "あああ";
            }
            else{
               print "失敗";
